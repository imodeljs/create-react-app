...
- Additional configuration options
  ...
  | USE_NEW_ASSET_COPY      | ✅ Used     | ✅ Used    | When set to `true`, uses the new copy static assets workflow which copies all files from dependencies starting with `@bentley` or `@itwin` scope. In addition it would make them available to both a `build` and `start`. Note: There is a bug with the update when used in conjunction with pnpm or yarn workspaces. |
